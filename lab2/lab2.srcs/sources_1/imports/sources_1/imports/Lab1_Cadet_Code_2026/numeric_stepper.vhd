-- Numeric Stepper: Holds a value and increments or decrements it based on button presses
-- C2C Jayden Randolph, 05Mar2026

library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use IEEE.NUMERIC_STD.ALL;

entity numeric_stepper is
  generic (
    num_bits  : integer := 11;
    max_value : integer := 1023;
    min_value : integer := -1024;
    delta     : integer := 10
  );
  port (
    clk     : in  std_logic;
    reset_n : in  std_logic;                    -- active-low synchronous reset
    en      : in  std_logic;                    -- enable
    up      : in  std_logic;                    -- increment on rising edge
    down    : in  std_logic;                    -- decrement on rising edge
    q       : out signed(num_bits-1 downto 0)   -- signed output
  );
end numeric_stepper;

architecture numeric_stepper_arch of numeric_stepper is
    signal process_q : signed(num_bits-1 downto 0) := to_signed((max_value + min_value)/2 ,num_bits);
    signal prev_up, prev_down : std_logic := '0';
    signal is_increment, is_decrement : boolean := false;
    signal up_debounced, down_debounced : std_logic := '0';
    
begin

debouncer_up : entity work.debouncer
    port map ( 
        clk => clk,
        reset_n => reset_n,
        btn_in => up,
        btn_out => up_debounced
    );
    
debouncer_down : entity work.debouncer
    port map ( 
        clk => clk,
        reset_n => reset_n,
        btn_in => down,
        btn_out => down_debounced
    );


is_increment <= true when ((en = '1') and (up_debounced = '1') and (prev_up = '0'))
    else false;
is_decrement <= true when ((en = '1') and (down_debounced = '1') and (prev_down = '0'))
    else false;

process(clk)

begin
    if(rising_edge(clk)) then
        if(reset_n = '0') then
            process_q <= to_signed((max_value + min_value)/2 ,num_bits); --resets q
            prev_up <= '0';
            prev_down <= '0';
        elsif((process_q < max_value - delta) and (is_increment)) then
            if(to_integer(process_q) >= max_value - delta) then     
                process_q <= to_signed(max_value, num_bits);
            else    
                process_q <= (process_q + delta);
                prev_up <= '0';
            end if;
        elsif((process_q > min_value + delta) and (is_decrement)) then
            if(to_integer(process_q) <= min_value + delta) then     
                process_q <= to_signed(min_value, num_bits);
            else
                process_q <= (process_q - delta);
                prev_down <= '1';
            end if;
        end if;
        
        prev_up <= up_debounced;
        prev_down <= down_debounced;
                       
    end if;
end process;

q <= process_q;

    
end numeric_stepper_arch;

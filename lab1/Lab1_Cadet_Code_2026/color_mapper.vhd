...
signal is_vertical_gridline, is_horizontal_gridline, is_within_grid, is_trigger_time, is_trigger_volt, is_ch1_line, is_ch2_line,
    is_horizontal_hash, is_vertical_hash, is_within_drawing_box_time, is_within_drawing_box_volt : boolean := false;
...
constant triangle_size : integer := 5;
...
--Creates drawing box for triggers
is_within_drawing_box_time <= true when ((to_integer(position.row) >= grid_start_row) and
                    (to_integer(position.row) <= (grid_start_row + triangle_size)))
                    else false;
                    
is_within_drawing_box_volt <= true when ((to_integer(position.col) >= grid_start_col) and
                    (to_integer(position.col) <= (grid_start_col + triangle_size)))
                    else false;                 

--Logic for is_trigger:
--0 <= abs(col - trigger.t) <= 5 - (row - 20)
--__ <= "within ____ of the trigger position" <= "makes 5x5 square" ("makes the square into a triangle")

is_trigger_time <= true when (is_within_grid and 
                   is_within_drawing_box_time and
                   (abs(to_integer(position.col) - to_integer(trigger.t)) < triangle_size - (to_integer(position.row) - grid_start_row)))
                   else false;                   
                   
is_trigger_volt <= true when (is_within_grid and 
                   is_within_drawing_box_volt and
                   (abs(to_integer(position.row) - to_integer(trigger.v)) < hash_size - (to_integer(position.col) - grid_start_col))) 
                   else false;
                 
--Turns on ch1/ch2  
is_ch1_line <= true when (is_within_grid and 
               (ch1.active = '1') and (ch1.en = '1')) else
               false;
is_ch2_line <= true when (is_within_grid and 
               (ch2.active = '1') and (ch2.en = '1')) else
               false;
               
--Uses the same logic as when creating gridlines, just cut of the lines
is_horizontal_hash <= true when (is_within_grid and
                      (((to_integer(position.col) - grid_start_col) mod hash_horizontal_spacing) = 0) and
                      (to_integer(position.row) <= (center_row + hash_size)) and 
                      (to_integer(position.row) >= (center_row - hash_size))) else
                      false;
is_vertical_hash <= true when (is_within_grid and
                    ...
                    (to_integer(position.col) >= (center_column - hash_size))) else
                    false;
...
color <= trigger_color when (is_trigger_time or is_trigger_volt) else
         ...
         gridline_color when (is_horizontal_gridline or is_vertical_gridline or is_horizontal_hash or is_vertical_hash) else
         background_color;
